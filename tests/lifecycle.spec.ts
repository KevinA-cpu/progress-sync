...
test('discarding one attempt keeps an unresolved capture in another tab deliverable', async ({
  extensionContext, progress, problem,
}) => {
  const { server } = await setup(extensionContext, progress);
  server.loseBeforeAt = 'ref';
  ...
  try {
    ...
    const pendingSource = `// Second tab\n${submittedSource}`;
    await second.getByRole('textbox', { name: 'Solution' }).fill(pendingSource);
    await second.getByRole('button', { name: 'Submit', exact: true }).click();
    ...
    expect([...server.files].filter(([path]) => path.endsWith('/solution.v')).map(([, source]) => source))
      .toEqual([pendingSource.replaceAll('\n', '\r\n')]);
    await stopExtensionWorker(extensionContext, progress);
    await progress.reload();
    await expect(progress.getByRole('status')).toHaveText('1 captured attempt.');
    await expect(progress.getByRole('textbox', { name: 'Submitted source (read-only)' })).toHaveValue(pendingSource);
  } finally {
    grading.resolve();
  }
});

test('a discarded attempt cannot let its resolved result document poison a newer same-frame submission', async ({
  extensionContext, progress, problem,
}) => {
  const { server } = await setup(extensionContext, progress);
  server.loseBeforeAt = 'ref';
  ...
  try {
    ...
    progress.once('dialog', dialog => dialog.accept());
    await progress.getByRole('button', { name: 'Discard local attempt', exact: true }).click();
    await expect(progress.getByRole('status'))
      .toHaveText('No captured attempts yet. Submit using the in-page HDLBits editor.');
    server.loseBeforeAt = null;
    const writes = server.writes.length;

    const newSource = `// Newer attempt\n${submittedSource}`;
    await problem.getByRole('textbox', { name: 'Solution' }).fill(newSource);
    await problem.getByRole('button', { name: 'Submit', exact: true }).click();
    await expect(progress.getByText('Waiting for the result - not saved to GitHub', { exact: true })).toHaveCount(1);
...
for (const loss of ['membership', 'suspended installation'] as const) {
  test(`failed explicit destination verification for ${loss} pauses previously eligible pending work`, async ({
    extensionContext, progress, problem,
  }) => {
    const { server, target, page } = await setup(extensionContext, progress);
    server.loseBeforeAt = 'ref';
    ...
    switch (loss) {
      case 'membership':
        target.included = false;
        error = 'The repository is not accessible to the selected App installation. Select it on GitHub, then verify again.';
        break;
      case 'suspended installation':
        target.suspended = true;
        error = 'Install this GitHub App on your personal account, then refresh installations.';
        break;
    }
    await page.getByRole('button', { name: 'Verify pending or saved repository' }).click();
